...
def test_r_rim_de_una_dolina_es_un_artefacto_del_ajuste():
    """Una dolina no tiene borde, asi que nada ancla r_rim y el optimizador
    aterriza donde sea.

    Medido sobre radios de 40 a 100 px, r_rim salta entre 23 y 219 px sin
    relacion con el tamano de la dolina, y el punto exacto depende de la
    version de scipy. Esta prueba fija esa INESTABILIDAD, que es el hecho
    real, en lugar de fijar un valor concreto.

    (Una version anterior de esta prueba afirmaba que la dolina de radio 80
    disparaba `edge_limited`. Eso era cierto solo con scipy 1.17 y fallaba en
    Colab. Fijar un resultado inestable no es una prueba: es una casualidad
    documentada.)
    """
    radios = [40.0, 60.0, 70.0, 80.0, 90.0, 100.0]
    ajustados = [morphometric_metrics(dolina_karstica(radio=r))["r_rim_px"]
                 for r in radios]
    assert max(ajustados) / min(ajustados) > 2.0, (
        "si r_rim fuera estable, seguiria al tamano de la dolina; "
        f"obtenidos: {[round(a, 1) for a in ajustados]}")


def test_dolina_no_pasa_como_crater_en_ningun_tamano():
    """Lo que SI es estable: ningun tamano de dolina cumple a la vez los dos
    criterios exigentes de un crater. Cae por coherencia, por circularidad, o
    por las dos."""
    for radio in (40.0, 60.0, 70.0, 80.0, 90.0, 100.0):
        m = morphometric_metrics(dolina_karstica(radio=radio))
        assert not (m["coherence"] >= 0.85 and m["bci"] >= 0.85), (
            f"la dolina de radio {radio} paso los dos criterios: "
            f"coher {m['coherence']:.2f}, BCI {m['bci']:.2f}")
